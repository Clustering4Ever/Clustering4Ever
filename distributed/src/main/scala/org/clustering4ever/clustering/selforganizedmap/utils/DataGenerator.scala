...
object DataGenerator extends Serializable
{
  ...
  {
    private def nextVector(i: Int) = pmodels(Random.nextInt(pmodels.length)).genVector
    private def nextNamedVector(i: Int) = pmodels(Random.nextInt(pmodels.length)).genNamedVector
