...
object Output extends Serializable
{
  ...
  def write(outputDir: String, datas: RDD[Array[Double]], model: AbstractModel, nbRowSOM:Int, nbColSOM: Int): String =
  {
      ...
      val protosString = prototypes.map( d => d.mkString(",")).mkString("\n")
      ...
      val card = prototypes.indices.map(d => if (sumAffectedDatas.contains(d)) sumAffectedDatas(d) + "" else "0" )
      ...
      val maps = prototypes.zip(card).map(d => d._1.mkString(",") + "," + d._2).mkString("\n")
