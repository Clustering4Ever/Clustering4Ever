...
class Matrix(val elements: Array[Array[Double]]) extends Serializable
{
  ...
  def += (other: Matrix): Matrix =
  {
    ...
    var ans = 0D
    var i = 0
    for (i <- elements.indices) for (j <- elements(i).indices) elements(i)(j) += other.elements(i)(j)
    this
  }
  ...
  override def toString: String =
  {
    ...
    for (row <- elements) for (elem <- row) str.append(" |").append("%.2f".format(elem))
    str.append("|\n")
    //elements.foreach{r =>
