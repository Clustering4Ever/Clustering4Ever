...
import org.clustering4ever.clustering.patchwork.PatchWorkUtils._
/**
...
final case class PatchWorkCell(val cellNumber: Array[Int]) extends Serializable {
  ...
  override def equals(o: Any) = o match {
    case that: PatchWorkCell => that.cellNumber.sameElements(this.cellNumber)
    case _ => false
  }
}
/**
...
class PatchWork(val epsilon: Epsilon, val minPts: Int, val ratio: Double, val minCell: Int) extends Serializable {
   ...
   * @return
   */
  def getNearCell(p: Array[Int]): List[Array[Int]] = p.length match {
  ...
  def createClusters(cardinalsPatchwork: List[(Array[Int], Int)], minPts: Int): List[PatchWorkCluster] = {
    ...
    cardinalsPatchwork.sortBy(_._2).foreach{ cell =>
      ...
      if (cell._2 >= minPts) {
        ...
        if (!clusterList.exists(cluster => cluster.cellsList.contains(cellKey))) {
