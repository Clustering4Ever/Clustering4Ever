...
import org.clustering4ever.clustering.stream.s2gstream.{PointObj, Prototype}
...
class BatchStreamModel(
  ...
  // point To Object
    ...
    val dataPart = e.take(e.length - labId) //labId=-2 because the 2 last columns represent labels & id
    ...
    val dataPart = e.take(e.length - 2) //-2 because the 2 last columns represent labels & id
    val part1 = Vector(dataPart.take(dim))
  ...
  def updateObj(rdd: RDD[PointObj], gstream: BatchStream, kk: Int, dim: Int): BatchStreamModel = {
    ...
    for( i <- this.errors.indices) this.errors(i) *= gstream.d
  ...
  def removeOldEdges(maxAge: Int) = {
    val delRowCol: mutable.ArrayBuffer[mutable.ArrayBuffer[Int]] = mutable.ArrayBuffer()
    	this.ages.indices.foreach(i => delRowCol += this.ages(i).zipWithIndex.collect{ case (v, idx) if v > maxAge => idx } )
  ...
  def addNewNodes(nbNodesToAdd: Int, kk: Int, dim: Int, alphaErr: Double) = {
      ...
      val f = this.errors.indexOf(this.errors.zipWithIndex.collect{ case (a, b) if qNeighbors.contains(b) => a }.max)
  ...
  def addPairwise[N: ClassTag](a: Array[N], b: Array[N])(implicit num: Numeric[N]): Array[N] = (a zip b).map { case (x, y) => num.plus(x, y) }
