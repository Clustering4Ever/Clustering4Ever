package org.clustering4ever.clustering.stream.s2gstream
...
class S2gstreamModel(
  ...
  def updateObj(
    ...
    for (i <- errors.indices) errors(i) *= gstream.d
 ...
 case (sum, (_, (_, _, sumPoint, _, _))) => sum + sumPoint
  ...
  def addNewNodes(nbNodesToAdd: Int, kk: Int, dim: Int, alphaErr: Double) = {
    for (j <- 1 until nbNodesToAdd) {
      ...
      val indexToLookFor = errors.zipWithIndex.collect{ case (a, idx) if qNeighbors.contains(idx) => a }.max
  ...
  def addElementLast(e: Int, a: mutable.ArrayBuffer[mutable.ArrayBuffer[Int]]) = {
    ...
    if (a.isEmpty) buffer += mutable.ArrayBuffer.fill(1)(e)
    else {
  ...
  def addPairwiseInt(a: Array[Int], b: Array[Int]): Array[Int] = {
    val output = Array.ofDim[Int](a.length)
    var i = 0
    while (i < a.length) {
      output(i) = a(i) + b(i)
      i += 1
