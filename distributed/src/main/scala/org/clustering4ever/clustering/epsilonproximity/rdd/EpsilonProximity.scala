...
import org.clustering4ever.clustering.FusionSmallerClusters
import org.clustering4ever.clustering.UtilSpark._
import org.clustering4ever.distances.{ContinuousDistance, Distance}
import org.clustering4ever.math.{Hashing, HashingScalar}
import org.clustering4ever.roottraits
import org.clustering4ever.roottraits.{Clusterizable, GVector, ScalarVector}
import org.clustering4ever.sparkcoreextension.{ClusteringAlgorithmDistributed, ClusteringModelDistributed}
import org.clustering4ever.util.SumVectors._
import org.clustering4ever.util.VectorsAddOperationsImplicits._
import org.clustering4ever.util.{DiscoverConnexComponents, SimilarityMatrix}
import org.clustering4ever.clustering.epsilonproximity.{EpsilonProximityScalar => EpsilonProximityScalarLocal}
 ...
 */
final case class EpsilonProximityModelScalar[D <: ContinuousDistance, Hash <: HashingScalar](val clusterizedVectorsSortedByID: RDD[(Long, (ScalarVector, Int))], val metric: D, val clustersNumber: Int, protected val inputDataHashCode: Int) extends EpsilonProximityModelAncestor[ScalarVector, D, Hash] {

	val algorithmID = roottraits.EpsilonProximityScalar

	protected[clustering4ever] def obtainClustering[O, Cz[Y, Z <: GVector[Z]] <: Clusterizable[Y, Z, Cz]](data: RDD[Cz[O, ScalarVector]])(implicit ct: ClassTag[Cz[O, ScalarVector]]): RDD[Cz[O, ScalarVector]] = {
...
final case class EpsilonProximityScalar[
	...
	final val algorithmID = roottraits.EpsilonProximityScalar
	...
	private[this] def applyLshAndDefinedEpsilon[O, Cz[Y, Z <: GVector[Z]] <: Clusterizable[Y, Z, Cz]](data: RDD[Cz[O, ScalarVector]]): (RDD[Cz[O, ScalarVector]], String) = {
		val nbDim = data.first.v.vector.length
		...
		val computedEpsilon = if(epsilonNature == "eps") epsilon
				...
				def definedKNNEpsilon = {
				    ...
				    val localityRDD = generateDataLocalityOnHashsedDS(lshizedRDD, bucketNumber, bucketLayers)
	...
	private[this] def exchangeDataBetweenPartitions[O, Cz[Y, Z <: GVector[Z]] <: Clusterizable[Y, Z, Cz]](clusterizedBDS: RDD[(immutable.Iterable[(PartitionIndex, ClusterID, mutable.ArrayBuffer[Cz[O, ScalarVector]])], EpsilonPerBucket)]): RDD[(Int, (Int, Int, mutable.ArrayBuffer[Cz[O, ScalarVector]]))] = {
	...
	private[this] def fusion2partitionsOfClusters[O, Cz[Y, Z <: GVector[Z]] <: Clusterizable[Y, Z, Cz]](epsilonAvg: Double, left: FusionableClusterizedData[O, Cz], right: FusionableClusterizedData[O, Cz]): mutable.HashSet[List[(Int, Int)]] = {
	...
	private[this] final def applyEpsilonClusteringMin[O, Cz[Y, Z <: GVector[Z]] <: Clusterizable[Y, Z, Cz]](data: RDD[Cz[O, ScalarVector]], cmin: Int)(implicit ct: ClassTag[Cz[O, ScalarVector]]): EpsilonProximityModelScalar[D, Hash] = {
		...
		val centroids = clusterizedRDD.map( cz => (cz.clusterIDs.last, cz.v) )
			.reduceByKeyLocally(sumVectors(_, _))
	...
	final def fit[O, Cz[Y, Z <: GVector[Z]] <: Clusterizable[Y, Z, Cz]](data: RDD[Cz[O, ScalarVector]])(implicit ct: ClassTag[Cz[O, ScalarVector]]): EpsilonProximityModelScalar[D, Hash] = {
		...
		else applyEpsilonClusteringMin(data, cmin)
	}
