...
class Regression(var dsXYTrain:Array[(Int, (Array[Double],Array[Double]))], var h:Int, var g:Int)(var allGroupedData:HashMap[Int,Int], var nbBloc:Int) extends Serializable
{
	...
	type DSPerClass = Array[(Int, (Array[Double], Array[Double], Int))]
	type ClassedDStype = Array[(Int, DSPerClass)]
	type IdXDs = Array[ArrayBuffer[(Int, Array[Double])]]
	type YDs = Array[ArrayBuffer[Array[Double]]]
	type RegPerClass = (Double, DenseMatrix[Double], Array[Double], Array[(Int, Array[Double])])
	type ClassedDsPerGrp = Array[(Int, Array[(Int, Int, Array[(Int, Int, Array[Double], Array[Double])])])]
	...
	def posInClassForMovingPoints(currClass:Int, elemNb:Int, limitsClass:Array[Int]) =
	{
		...
		{
			elemNb - limitsClass( currClass - 1 ) - 1
		}
	}

	def removeFirstElemXY(_class:Int, xDS: IdXDs, yDS: YDs) =
	{
		xDS(_class).remove(0)
		yDS(_class).remove(0)
	}

	val prepareMovingPoint = (classedDS: ClassedDStype, xDS: IdXDs, yDS: YDs, g:Int, elemNb:Int, currClass:Int, limitsClass:Array[Int]) =>
	{
				...
				//val classPosByID = HashMap.empty[Int, (Int, Int)]
		  		...
		  		breakable
		  		{
				  	...
				  	while( continue && nbIte != stop )
				  	{
					  	...
					  	val regPerClass = for( i <- rangeOverClasses ) yield( Pls.prepareAndLaunchPls(inputX, inputY, i, h) )
					  	...
					  	val regPerClass2 =
					  		try for( i <- rangeOverClasses ) yield( Pls.prepareAndLaunchPls(inputX, inputY, i, h) )
			  	...
			  	else
			  	{
					...
					regPerClassFinal = for( i <- rangeOverClasses ) yield( Pls.prepareAndLaunchPls(inputX, inputY, i, h) )
