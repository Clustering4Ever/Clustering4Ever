...
final case class EpsilonProximityScalar[
	...
	private[this] final def applyClusteringPerBucket[O, Cz[Y, Z <: GVector[Z]] <: Clusterizable[Y, Z, Cz]](lshizedRDD: RDD[Cz[O, ScalarVector[V]]], computedEpsilon: String): RDD[(immutable.Iterable[(Int, Int, mutable.ArrayBuffer[Cz[O, ScalarVector[V]]])], Double)] = {
		lshizedRDD.mapPartitionsWithIndex{ (idxPart, it) =>
		    ...
		    val epsilonLocalModel = EpsilonProximityScalarLocal(computedEpsilon, metric).run(data)
	...
	final def run[O, Cz[Y, Z <: GVector[Z]] <: Clusterizable[Y, Z, Cz]](data: RDD[Cz[O, ScalarVector[V]]])(implicit ct: ClassTag[Cz[O, ScalarVector[V]]]): EpsilonProximityModelScalar[V, D, Hash] = {
...
object EpsilonProximity
{
	final def run[
		...
		EpsilonProximityScalar(epsilon, fusionClustersMethod, metric, bucketNumber, hasher, cmin, bucketLayers, storageLevel, divisionFactor).run(data)
