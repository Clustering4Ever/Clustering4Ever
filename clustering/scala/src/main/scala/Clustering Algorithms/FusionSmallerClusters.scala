...
trait FusionSmallerClusters extends ClusteringSharedTypes {
	 ...
	 *
	 */	
	private[this] def selectSmallerClusterIDs[Cardinality](clusterCardinalities: scala.collection.Map[ClusterID, Cardinality], cmin: Int)(implicit num: Numeric[Cardinality]) = {
		clusterCardinalities.filter{ case (clusterID, cardinality) => num.toLong(cardinality) < cmin }.keys.toBuffer
	}
	/**
	 *
	 */	
	private[this] def selectSmallerCluster[Cardinality, V <: GVector[V]](toGatherCentroids: GenSeq[(ID, NewClusterID, V, Cardinality, ClusterID)], cmin: Int)(implicit num: Numeric[Cardinality]) = {
		mutable.ArrayBuffer(toGatherCentroids.filter{ case (_, _, _, cardinality, _) => num.toLong(cardinality) < cmin }.seq:_*)
	}
	/**
	 *
	 */	
	private[this] def obtainToGatherCentroids[Cardinality, V <: GVector[V]](centroids: GenSeq[(Int, V)], clusterCardinalities: scala.collection.Map[ClusterID, Cardinality]) = {
		centroids.zipWithIndex.map{ case ((clusterID, vector), id) => (id, clusterID, vector, clusterCardinalities(clusterID), clusterID) }
	}
	/**
	 *
	 */	
	private[this] def settingUpFusionAlgorithm[Cardinality: Numeric, V <: GVector[V]](centroids: GenSeq[(Int, V)], clusterCardinalities: scala.collection.Map[ClusterID, Cardinality], cmin: Int) = {
	 ...
	 *
	 */	
	private[this] def findClosestCentroidWithAnotherClusterID[Cardinality, V <: GVector[V]](sortedClosestCentroid: Seq[(Int, NewClusterID, V, Cardinality, Int)], currentClusterID: Int) =
		sortedClosestCentroid.find{ case (_, newClusterID, _, _, _) => newClusterID != currentClusterID }.get
	/**
	 *
	 */	
	private[this] def filterOthersCentroidWithSameClusterID[Cardinality, V <: GVector[V]](sortedClosestCentroid: Seq[(Int, NewClusterID, V, Cardinality, Int)], closestClusterID: Int) =
		sortedClosestCentroid.filter{ case (_, newClusterID, _, _, _) => newClusterID == closestClusterID }
	/**
	 *
	 */	
	private[this] def settingUpBuffers[Cardinality, V <: GVector[V]](
	 ...
	 *
	 */	
	private[this] def obtainIDsOfCentroidToUpdate[Cardinality: Numeric, V <: GVector[V]](toGatherCentroids: Array[(ID, NewClusterID, V, Cardinality, ClusterID)], oldClusterIDs: mutable.HashSet[Int]) = {
		toGatherCentroids.collect{ case (id, newClusterID, _, _, _) if oldClusterIDs.contains(newClusterID) => id }
	}
	/**
	 * @param centroids: Seq of centroids with their ID and vector prototype
	 ...
	 **/
	def fusionSmallerClusters[Cardinality, V <: GVector[V], D <: Distance[V]](centroids: GenSeq[(ClusterID, V)], clusterCardinalities: scala.collection.Map[ClusterID, Cardinality], cmin: Int, metric: D)(implicit num: Numeric[Cardinality]): (immutable.Map[Int, Int], immutable.Map[Int, Cardinality]) = {
	 ...
	 *
	 */
	def obtainDataToFusionLittleCluster[N: Numeric](clusterizedDS: GenSeq[(Int, (Int, Seq[N]))]): (GenMap[Int, Seq[N]], GenMap[Int,Int]) = {
	 ...
	 *
	 */
	def fusionLittleClustersIn[V <: GVector[V], D <: Distance[V]](labelizedData: GenSeq[(Int, (Int, V))], cmin: Int, metric: D, isRealVectorElseBinary: Boolean): GenSeq[(Int, (Int, V))] = {
		if(cmin == 0) labelizedData
			...
			val centroids = clusters.map{ case (clusterID, aggregates) =>
				(
					...
					{
						val vectors = aggregates.map{ case (_, (_, vector)) => vector }
						(if(isRealVectorElseBinary) ClusterBasicOperations.obtainMean(vectors.asInstanceOf[GenSeq[Seq[Double]]]) else ClusterBasicOperations.obtainMode(vectors.asInstanceOf[GenSeq[Seq[Int]]])).asInstanceOf[V]
					}
				)
			}.toSeq.par
			val (newClusteringMapping, _) = if( isRealVectorElseBinary ) fusionSmallerClusters[Long, V, D](centroids, clustersCardinalities, cmin, metric) else fusionSmallerClusters[Long, V, D](centroids, clustersCardinalities, cmin, metric)
...
object FusionSmallerClusters extends FusionSmallerClusters {	
	 ...
	 * Fusion clusters with cardinality under cmin to their closest aggregate of cluster looking on each cluster prototype to make its decision
	 */
	def fusionSmallerCluster[Cardinality: Numeric, V <: GVector[V], D <: Distance[V]](centroids: GenSeq[(ClusterID, V)], clusterCardinalities: scala.collection.Map[ClusterID, Cardinality], cmin: Int, metric: D): (immutable.Map[Int, Int], immutable.Map[Int, Cardinality]) = {
		fusionSmallerClusters[Cardinality, V, D](centroids, clusterCardinalities, cmin, metric)
	}
	/**
	 * Fusion clusters with cardinality under cmin to their closest aggregate of cluster looking on each cluster prototype to make its decision
	 */
	def fusionLittleClusters[V <: GVector[V], D <: Distance[V]](labelizedData: GenSeq[(ClusterID, (Int, V))], cmin: Int, metric: D, isRealVectorElseBinary: Boolean): GenSeq[(Int, (Int, V))] = {
		fusionLittleClustersIn[V, D](labelizedData, cmin, metric, isRealVectorElseBinary)
	}
}