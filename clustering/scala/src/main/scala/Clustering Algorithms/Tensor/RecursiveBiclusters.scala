...
class RecursiveBiclusters ( val l1: Array[Int], 
  ...
  def run(data: mutable.ArrayBuffer[DenseMatrix[Double]]) = {
   ...
   columnMatrix = matrice1(tensor1, timeColumn, columnMatrix, 0, 0, 0 )
    ...
    def annulation(t:mutable.ArrayBuffer[DenseMatrix[Double]],jf: Array[Int], js: Array[Int]): 
    ...
    val tensor_remain = annulation(tensor1, row, column)
  ...
  //  var nombre = 0
    ...
    for (nombre <- 0 until l1.length) {
       r1 += one_bicluster(l1(nombre), l2(nombre), r1(nombre))
      }   
  }
}  
