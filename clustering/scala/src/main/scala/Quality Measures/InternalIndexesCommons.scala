...
object InternalIndexesDBCommons {
   ...
   */
  def good[S[Double] <: Seq[Double]](scatter1: Double, scatter2: Double, center1: S[Double], center2: S[Double], metric: ContinuousDistance[S[Double]]): Double = (scatter1 + scatter2) / metric.d(center1, center2)
   ...
   */
  def scatter[S[Double] <: Seq[Double]](cluster: Seq[S[Double]], centroid: S[Double], metric: ContinuousDistance[S[Double]]): Double = cluster.map(metric.d(centroid, _)).sum / cluster.size
}