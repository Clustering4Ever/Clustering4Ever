...
import scala.math.{min, max, sqrt, pow, Pi, abs, log, exp}
import scala.collection.{GenSeq, mutable}
import scala.util.Random
import scala.util.Try
import scala.reflect.ClassTag
import breeze.linalg.{DenseVector, DenseMatrix, cov, inv, pinv, det, norm}
...
class GaussianMixture[O, Cz[B, C <: GVector[C]] <: Clusterizable[B, C, Cz]](
	...
	def run(data: GenSeq[Cz[O, ScalarVector]]) = {
		...
		val vectorizedDataset: GenSeq[DenseVector[Double]] = data.map( cz => new DenseVector(cz.v.vector.toArray))
