 ...
 */
class ScalaInternalsIndexes extends RealScalaDatasets
{
  /**
  ...
  **/
  private[this] def good(scatter1: Double, scatter2: Double, center1: Array[Double], center2: Array[Double], metric: ContinuousDistances) = (scatter1 + scatter2) / metric.distance(center1, center2)
   ...
   **/
  private[this] def scatter(cluster: Array[Array[Double]], centroid: Array[Double], metric: ContinuousDistances) = ( for(p <- cluster) yield(metric.distance(centroid, p)) ).reduce(_ + _) / cluster.size
  ...
  {  
    ...
    def aiList(cluster:Array[(Int, Array[Double])]) =
    {
      ...
      val allPointsDistances = for( pp <- pointPairs ) yield( ((pp._1._1, pp._2._1), metric.distance(pp._1._2, pp._2._2)) )
    ...
    def sk(testedLabel:Int) =
    {
      ...
      val allDistances = cart.map{ case (((_, vector1), id1), ((clusterID2, vector2), _)) => ((id1, clusterID2), metric.distance(vector1, vector2)) }.groupBy(_._1).map{ case (k,v)=> (k, v.map(_._2).reduce(_ + _)) }.toArray
...
object ScalaInternalsIndexes extends RealScalaDatasets
{
  ...
  {
    (new ScalaInternalsIndexes).daviesBouldinIndexInside(data, clusterLabels, metric)
  }
  ...
  def silhouette(clusterLabels: Array[Int], data: Array[(Int, Array[Double])], metric: ContinuousDistances) =
  {
    (new ScalaInternalsIndexes).silhouette(clusterLabels, data, metric)
  }
