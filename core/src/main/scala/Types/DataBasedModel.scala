...
class DataBasedModel[Obj, ID](val data: mutable.HashMap[Int, mutable.HashSet[(ID, Obj)]], metric: Distance[Obj]) extends ClusteringModel
{
	...
	def knnPredict(obj: Obj, k: Int): Int = dataAsSeq.sortBy{ case (_, (_, obj2)) => metric.d(obj, obj2) }.take(k).map(_._1).groupBy(identity).maxBy{ case (clusterID, aggregate) => aggregate.size }._1
