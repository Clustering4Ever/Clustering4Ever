...
import org.clustering4ever.math.distances.{RealClusterizableDistance, ContinuousDistance, RawContinuousDistance, Distance}
 ...
 * The Euclidean distance with or without squareRoot
 */
class Euclidean[V <: Seq[Double]](final val squareRoot: Boolean = true, val id: MetricID = 0) extends EuclideanMeta[V] with ContinuousDistance[V] {
	/**
	  * The Euclidean distance with or without squareRoot
	  * @return The Euclidean distance between dot1 and dot2
	  */
	...
	/**
	  ...
	  * @return The Euclidean distance between dot1 and dot2
	  */
	def d2(dot1: V, dot2: V): Double = euclidean(dot1, dot2)
	/**
	 *
	 */
	 ...
	 *
	 */
	def norm2(dot: V): Double = SumVectors.euclideanNorm(dot)
