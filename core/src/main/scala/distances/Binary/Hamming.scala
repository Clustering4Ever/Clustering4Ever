...
class HammingClusterizable[ID: Numeric, Obj, V <: Seq[Int]](val classicalMetric: Hamming[V]) extends HammingMeta with BinaryClusterizableDistance[BinaryClusterizable[ID, Obj, V], V] {
	/**
	  * The Hamming distance with or without squareRoot
	  * @return The Hamming distance between dot1 and dot2
	  */
