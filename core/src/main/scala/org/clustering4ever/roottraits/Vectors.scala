 ...
 * @author Beck Gaël
 */
import org.clustering4ever.util.{SumVectors, VectorsAddOperationsImplicits}
...
final case class MixedVector(final val binary: Array[Int], final val scalar: Array[Double]) extends GMixedVector[MixedVector] {
	...
	final def pickFeatures(indices: Int*): MixedVector = {
			val (binaries, scalars) = indices.partition(_ < binary.length)
			...
			scalars.foreach( i => scaBuilder += scalar(i - binary.length - 1) )
	...
	final def toScalarVector: ScalarVector = {
		...
		builder.sizeHint(scalar.length + binary.length)
