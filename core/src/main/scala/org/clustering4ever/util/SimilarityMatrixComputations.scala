 ...
 * @author Beck Gaël
 */
import org.clustering4ever.distances.{Distance, GenericDistance}
import org.clustering4ever.roottraits.{GVector, Preprocessable}
...
final case class HalfSimilarityMatrix(val matrix: Array[Array[Double]]) {
	...
	def apply(x: Int, y: Int): Double = {
		require(x < matrix.length)
		require(y < matrix.head.length)
		if (x > y) matrix(x)(y)
		else matrix(y)(x)
	}
	/**
	...
	def distanceFromIdx(idx: Int): Array[Double] = {
		...
		val rowComplement = (idx + 1 until matrix.length).map(i => matrix(i)(idx) ).toArray
		buff ++= incompleteRow
		buff += 0D
		buff ++= rowComplement
		buff.toArray
