package org.clustering4ever.clustering.kfamily.jenksnaturalbreaks
...
final case class JenksNaturalBreaks(desiredNumberCategories: Int) extends ClusteringAlgorithm {
  ...
  final val algorithmID = roottraits.JenksNaturalBreaks
  /**
   ...
   */
  final def fit[@specialized(Int, Double, Long, Float) N](data: GenSeq[N])(implicit num: Numeric[N]): JenksNaturalBreaksModel[N] = {
...
object JenksNaturalBreaks {
  ...
  final def fit[@specialized(Int, Double, Long, Float) N](data: GenSeq[N], desiredNumberCategories: Int)(implicit num: Numeric[N]): JenksNaturalBreaksModel[N] = JenksNaturalBreaks(desiredNumberCategories).fit(data)

  final def apply[@specialized(Int, Double, Long, Float) N](data: GenSeq[N], desiredNumberCategories: Int)(implicit num: Numeric[N]): JenksNaturalBreaksModel[N] = fit(data, desiredNumberCategories)
