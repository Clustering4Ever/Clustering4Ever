...
object UMAP {
  ...
  private[clustering4ever] def nearestNeighbors(
    ...
    val nbTrees = 5 + round(pow(data.length, 0.5) / 20D).toInt
    val nbIters = max(5, round(log2(data.length)).toInt)
  ...
  /**
    ...
    */
  private[clustering4ever] def membershipStrengths(
    knnIndices: DenseMatrix[Int],
    knnDists: DenseMatrix[Double],
    sigmas: DenseVector[Double],
    rhos: DenseVector[Double]
  ): (DenseVector[Int], DenseVector[Int], DenseVector[Double]) = {
  ...
  private[clustering4ever] def optimizeLayout(
    ...
    val dim = headEmbedding.head.length
    val moveOther: Boolean = headEmbedding.length == tailEmbedding.length
    ...
    val cst1 = -2D * a * b
    val cst2 = 2D * gamma * b
    val rangeDim = (0 until dim).toArray

    def fillEpochSamples(n: Int, i: Int, alpha: Double, j: Int, k: Int): Unit = {
  ...
  /** Perform a fuzzy simplicial set embedding, using a specified
    ...
    *         euclidean space.
    */
  private[clustering4ever] def simplicialSetEmbedding(
    graph: DenseMatrix[Double],
    nComponents: Int,
    initialAlpha: Double,
    a: Double,
    b: Double,
  ...
  private[clustering4ever] def fuzzySimplicialSet(
    ...
    verbose: Boolean = false
      ...
      val result = Utils.makeMatrix(rows, cols, vals, data.length, data.length)
  ...
  /** Under the assumption of categorical distance for the intersecting
    ...
    */
  private[clustering4ever] def fastIntersection(simplicialSet: DenseMatrix[Double], target: Array[Double], unknownDist: Double = 1D, farDist: Double = 5D): DenseMatrix[Double] = {
  ...
  private[clustering4ever] def initTransform(indices: DenseMatrix[Int], weights: DenseMatrix[Double], embedding: Array[Array[Double]]): DenseMatrix[Double] = {
      ...
      val embedingDim = embedding.head.length
...
final case class UMAP(
  ...
  val verbose: Boolean = false
    ...
    def fit(xData: Array[Array[Double]], y: Option[Array[Double]] = None): Option[UMAPModel] = {
      ...
      val (oneRow, nNeighbors) = if (xData.length <= nNeighborsIn) {
          val oneRowIn = if (xData.length == 1) true else false
          (oneRowIn, xData.length - 1)
      }
      ...
      if (!oneRow) {
        ...
        /* Search Graph */
        val sgAux = new DenseMatrix[Int](xData.length, xData.length)
        ...
        val graph: Option[DenseMatrix[Double]] = if (y.isDefined) {
            ...
            tm match {
                case "categorical" => {  
                  ...
                  Some(UMAP.categoricalSimplicialSetIntersection(g, y.get, farDist = fd))
                }
                case _ => {  
...
case class UMAPModel(
  ...
  val initialAlpha: Double
    ...
    def transform(xData: Array[Array[Double]]): Array[Array[Double]] = {
        if (embedding.length <= 1) throw new IllegalArgumentException("Transform unavailable with only a single data sample")
        ...
        val graph = Utils.makeMatrix(rows, cols, vals, xData.length, learnedData.length)
        ...
        val inds = cols.toDenseMatrix.reshape(nNeighbors, xData.length).t
        val weights = wei.toDenseMatrix.reshape(nNeighbors, xData.length).t
