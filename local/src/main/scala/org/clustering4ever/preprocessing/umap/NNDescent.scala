...
object NNDescent {
        ...
        val initFromRandom: InitFromRandom = (nn, data, qp, h, rngState) => {
            ...
            def go(i: Int): Unit = {
                ...
                if (i < qp.length) {
                    ...
                    val indices: Array[Long] = Utils.rejectionSample(nn, data.length, rngState)
    ...
    def initialiseSearch(forest: Forest, data: Array[Array[Double]], queryPoints: Array[Array[Double]], nNeighbors: Int, rngState: Array[Long], dist: Distance): Heap = {
        ...
        val results = Heap(queryPoints.length, nNeighbors)
