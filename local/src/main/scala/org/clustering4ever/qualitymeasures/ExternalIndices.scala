...
import org.clustering4ever.roottraits.NmiNormalizationNature
...
trait ContingencyTableLocalCommons extends Serializable {
   ...
   *
   */
   val targetClassesNumber: Int
  /**
   *
   */
  private[qualitymeasures] val predClassesNumber: Int
  /**
   *
   */
  final private[qualitymeasures] lazy val emptyContingencyTable: Array[Array[Long]] = Array.fill(targetClassesNumber)(Array.fill(predClassesNumber)(0L))
  /**
   *
   */
   ...
   *
   */
  final protected[qualitymeasures] def putInContingencyTable(contingencyTable: Array[Array[Long]], targetAndPred: (Int, Int)): Array[Array[Long]] = {
   ...
   *
   */
  final protected[qualitymeasures] def fusionContingencyTables(contingencyTable1: Array[Array[Long]], contingencyTable2: Array[Array[Long]]): Array[Array[Long]] = {
...
trait BinaryExternalIndicesAncestor extends TPTNFPFNAncestor {
	...
	private[qualitymeasures] final type TP = Long

	private[qualitymeasures] final type FP = Long

	private[qualitymeasures] final type TN = Long

	private[qualitymeasures] final type FN = Long

	private[qualitymeasures] lazy final val tpCount = (1L, 0L, 0L, 0L)

	private[qualitymeasures] lazy final val tnCount = (0L, 1L, 0L, 0L)

	private[qualitymeasures] lazy final val fpCount = (0L, 0L, 1L, 0L)

	private[qualitymeasures] lazy final val fnCOunt = (0L, 0L, 0L, 1L)

	private[qualitymeasures] final def fillBinaryConfusionMatrix(truth: Int, pred: Int): (TP, FP, TN, FN) = {
...
trait MultiClassesContingencyTable extends ContingencyTableLocalCommons with TPTNFPFNAncestor {
   ...
   *
   */
  private[qualitymeasures] final lazy val tpfp: Long = sumPredColumnsCM.aggregate(0L)( (s, v) => s + MathC4E.binom(v, 2L), (sum1, sum2) => sum1 + sum2 )
  /**
   *
   */
  private[qualitymeasures] final lazy val tpfn: Long = sumTargetRowCM.aggregate(0L)( (s, v) => s + MathC4E.binom(v, 2L), (sum1, sum2) => sum1 + sum2 )
  /**
   *
   */
  private[qualitymeasures] final lazy val columnSize: Int = contingencyTable.head.length
  /**
   *
   */
  protected[qualitymeasures] lazy val sumOfChoose2ContingencyTable: Long = {
    @annotation.tailrec
    ...
    go(contingencyTable.length - 1, columnSize - 1, 0L)
  }
  ...
  final lazy val tn: Long = {
    ...
    val computeSumContingencyTable: Long = {
      ...
      go(contingencyTable.length - 1, columnSize - 1, 0)
    }
